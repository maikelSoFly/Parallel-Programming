...
void heat_part ( int n, int p, int id, double x_min, double x_max )
{
  ...
  for ( j = 1; j <= j_max; j++ )
  {
    ...
    if ( id < p - 1 )
    {
      MPI_Send ( &h[n+1], 1, MPI_DOUBLE, id+1, tag, MPI_COMM_WORLD );
    }
  ...
  int msg = 1;
  if (id != 0) {
      MPI_Recv ( &msg, 1, MPI_DOUBLE, id-1, tag, MPI_COMM_WORLD, &status );
  }
  ...
  if (id != p-1) {
      MPI_Send ( &msg, 1, MPI_DOUBLE, id+1, tag, MPI_COMM_WORLD );
  }
